...
public class CompanyParticipantController {
    ...
    public ResponseEntity<CompanyParticipantResponseDTO> applyToCompanyPost(@RequestBody CompanyParticipantRequestDTO requestDTO, @AuthenticationPrincipal UserDetails userDetails) {
    ...
    @PreAuthorize("@securityService.isActiveAndNotNoneUser() and hasAnyRole('ACTIVE')")
    public ResponseEntity<CompanyParticipantResponseDTO> updateParticipantStatus(@PathVariable Long participantId, @RequestParam CompanyParticipantStatus status,  @AuthenticationPrincipal UserDetails userDetails) {
        CompanyParticipantResponseDTO responseDTO = companyParticipantService.updateStatus(participantId, status);
        return ResponseEntity.ok(responseDTO);
    }
}
