...
public class HomeController {
    ...
    public ResponseEntity<?> getInfoBoardList(
            @AuthenticationPrincipal UserDetails userDetails
    ) {
        User user = securityService.getUserByUserDetails(userDetails);

        return ResponseEntity.ok(homeService.getInfoBoardList(user));
    }
    ...
    public ResponseEntity<?> getFreeBoardList(
        ...
        return ResponseEntity.ok(homeService.getFreeBoardList(user));
    }
    ...
    @PreAuthorize("@securityService.isNotTemporaryUser()")
    public ResponseEntity<?> getCompanyBoardList(
            @AuthenticationPrincipal UserDetails userDetails
    ) {
        User user = securityService.getUserByUserDetails(userDetails);

        return ResponseEntity.ok(homeService.getCompanyBoardList(user));
    }
